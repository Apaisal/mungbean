...
#
        ...
        if num_pixel > 3200:
            ...
            seq_chaincode = cv.FindContours(gray_seed, cv.CreateMemStorage(), cv.CV_RETR_LIST, cv.CV_CHAIN_CODE, (0, 0))
            ...
            for x, y in seq_b:
                ...
                i, j = ellipticFS(seq_chaincode, 4, n)
                n += 1
                real.append(i)
                imag.append(j)
...
#            cv.Zero(touching_grain)
            ...
            cv.ShowImage("traverse link", touching_grain)
#                cv.WaitKey(5)

#            comp = np.array([real, imag])
            plt.subplot(111)
            plt.plot(real, 'b-', imag, 'r-')
            plt.grid(True)
            plt.show()
